...
- 每次扩容增加 2 行 x 4 列
- 费用指数增长：$500M x 1.8^N
- 无扩容上限，但后期费用极高（第5次已超 $9B）
- 点击按钮弹出确认窗口，显示当前/扩容后规模和费用明细

### 6. 技术研发

点击"研发技术"，选择需要研发的技术。技术按三层依赖树排列，覆盖训练全流程（共 24 项）：
...
**GPU 分配**：按型号分配训练 GPU，滑动参数滑块时各型号最少需求数自动按算力折算联动
...
- 部署需为模型分配推理 GPU（按型号折算最少需求，以 H100 为基准）
- 部署后推理 GPU 被占用，不可用于训练或拆除
- 已部署模型产生每日 API 收入（受 DAU、token 量、开源/闭源、收入加成技术和部署 GPU 充足度影响）
- 可随时"下架"模型释放推理 GPU
- 部署后若供电超载同样触发断电 3 天
...
- 暂停 / 1x / 2x：控制游戏速度
- 自动存档：每 100 真实秒自动保存一次（页面刷新/关闭前也会自动保存）
