...
| H100 | Hopper | 990 | 80GB HBM3 | 700W | $30K | $200M |
| H800 | Hopper | 750 | 80GB HBM3 | 700W | $35K | $300M |
| MI300X | CDNA3 | 1307 | 192GB HBM3 | 750W | $20K | $300M |
| H200 | Hopper | 2000 | 141GB HBM3e | 700W | $45K | $500M |
| Gaudi 3 | Gaudi | 1835 | 128GB HBM2e | 900W | $35K | $600M |
| MI325X | CDNA3 | 1630 | 256GB HBM3e | 750W | $28K | $800M |
| B200 | Blackwell | 2250 | 192GB HBM3e | 1000W | $55K | $1B |
| B300 | Blackwell Ultra | 2500 | 288GB HBM3e | 1400W | $70K | $2B |
| MI355X | CDNA4 | 3000 | 288GB HBM3e | 1000W | $60K | $3B |
| GB300 NVL72 | Blackwell Ultra | 3750 | 288GB HBM3e | 1800W | $90K | $5B |
...
评分公式：
```
基础分 = 18 + max(0, log10(params) - 9) × 5.5
分类得分 = 基础分 × 通用质量 × 数据分布加成 × 技术加成 × 中断惩罚
最终得分 = Σ(分类得分 × 权重)
```
...
- 统一所有模态框通过右上角固定叉叉关闭
- 删除所有模态框底部的"关闭"和"取消"按钮
- 关闭按钮独立于模态框内容，不会被 innerHTML 覆盖

### v0.1.2 (2026-08-10)

- 修复 benchmark.js 文件截断问题
- 优化基准测试算法：通用质量加成上限从 1.30 提升至 1.55
- 添加 Tier 5 技术的基准测试分类加成（量子机器学习、AGI 对齐等）
- 升级研究员解锁难度：高级研究员 $1B、首席研究员 $5B
- 修复 liquid_cooling 描述与实际效果不匹配问题
- 所有源文件添加版权声明（作者：mukunjin）

### v0.1.1 (2026-08-10)

- 数据中心改为多层楼层系统（每层 400 个机架位）
- 添加 4x 加速支持
- 参数上限提升至 3TB
- 技术树改为 5 层，必须解锁当前层全部技术才能进入下一层
- 改为单存档系统

### v0.1.0 (2026-08-10)

- 初始版本
- 14 款 GPU 型号（A100 到 Vera Rubin）
- 41 项技术研发
- 三阶段训练系统
- 6 大类基准测试