...
const SaveSystem = {
  ...
  createSlot(name) {
    ...
    // 每个新存档独立完成新手引导；从这里触发可覆盖新开局与“另存为”。
    if (Game.state.running && typeof UI !== 'undefined') {
  ...
  resetGame() {
    ...
    // 重置数据采集
    DataCollection.state = { sources: {}, collected: false, totalTokens: 0, avgQuality: 0, activeJobs: [] };
    DataCollection.init();
